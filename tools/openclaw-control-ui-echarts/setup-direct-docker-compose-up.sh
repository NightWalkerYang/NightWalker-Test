...
inject_echarts_view_public_bootstrap() {
  local index_path="$1"
  local python_bin
  local python_index_path

  python_bin="$(resolve_python)"
  python_index_path="$(prepare_path_for_windows_python "$index_path")"

  "$python_bin" - "$python_index_path" <<'PY'
import pathlib
import re
import sys
...
inject_lufeng_public_bootstrap() {
  ...
  "$python_bin" - "$python_index_path" "$token" <<'PY'
import pathlib
import re
import sys
...
prepare_path_for_windows_python() {
  local input_path="$1"
...
resolve_gateway_image_ref() {
  ...
  local compose_image=""
  compose_image="$(
    "$DOCKER_BIN" compose config 2>/dev/null | awk '
      $1 == "openclaw-gateway:" {
        in_service = 1
        next
      }
...
ensure_gateway_image_available() {
  ...
  if [[ "$image_ref" == "openclaw:local" ]]; then
    ...
    "$DOCKER_BIN" build -t "$image_ref" -f Dockerfile .
  else
    printf '%s\n' "Image $image_ref is not available locally; pulling it now..." >&2
    "$DOCKER_BIN" pull "$image_ref"
  fi

  "$DOCKER_BIN" image inspect "$image_ref" >/dev/null 2>&1 || fail "Could not resolve a local openclaw-gateway image."
}

stage_tenant_platform_runtime_dependencies() {
  mkdir -p "$TENANT_PLATFORM_RUNTIME_DIR"
  cat >"$TENANT_PLATFORM_RUNTIME_DIR/package.json" <<'EOF'
{
  "name": "openclaw-tenant-platform-runtime",
  "private": true
}
EOF
  rm -rf "$TENANT_PLATFORM_RUNTIME_NODE_MODULES_DIR"
  ...
  if command -v npm >/dev/null 2>&1; then
    npm install \
      ...
      --no-package-lock \
      --no-save \
      --prefix "$TENANT_PLATFORM_RUNTIME_DIR" \
      "${TENANT_PLATFORM_RUNTIME_EXTRA_PACKAGES[@]}"
  else
    local image_ref
    local user_args=()
    ...
    "$DOCKER_BIN" run --rm \
      ...
      --no-package-lock \
      --no-save \
      --prefix tools/openclaw-control-ui-echarts/generated/tenant-platform-runtime \
      "${TENANT_PLATFORM_RUNTIME_EXTRA_PACKAGES[@]}"
  fi

  [[ -f "$TENANT_PLATFORM_RUNTIME_NODE_MODULES_DIR/pg/package.json" ]] || \
...
sync_gateway_control_ui_root() {
  ...
  if ! (cd "$ROOT_DIR" && "$DOCKER_BIN" compose config >/dev/null 2>&1); then
...
run_targeted_compose_up() {
  ...
  if cd "$ROOT_DIR" && "$DOCKER_BIN" compose up -d --force-recreate "${COMPOSE_UP_SERVICES[@]}"; then
...
extract_offline_vendors() {
  ...
  local vendor_dir="$2"
  local python_bin
  local python_bundle_path
  local python_vendor_dir

  python_bin="$(resolve_python)"
  python_bundle_path="$(prepare_path_for_windows_python "$bundle_path")"
  python_vendor_dir="$(prepare_path_for_windows_python "$vendor_dir")"
  mkdir -p "$vendor_dir"

  "$python_bin" - "$python_bundle_path" "$python_vendor_dir" <<'PY'
import json
import pathlib
import re
import sys
...
resolve_source_dir() {
  ...
  command -v "$DOCKER_BIN" >/dev/null 2>&1 || fail "docker is required when dist/control-ui is not present on the host."
  ...
  container_id="$("$DOCKER_BIN" create "$image_ref")"
  "$DOCKER_BIN" cp "$container_id:/app/dist/control-ui/." "$extracted_dir"
  "$DOCKER_BIN" rm -f "$container_id" >/dev/null 2>&1 || true
  container_id=""
