...
import {
  ...
  // Kingdee
  upsertKingdeeConnection,
  getKingdeeConnection,
  getKingdeeConnectionWithSecret,
  markKingdeeConnectionTested,
  getKingdeeQueryCache,
  setKingdeeQueryCache,
  // Dashboards
  createDashboard,
  getDashboard,
  getDashboardByPublicToken,
  listDashboards,
  updateDashboard,
  setDashboardPublic,
  deleteDashboard,
...
import {
  authenticate as kingdeeAuthenticate,
  queryBillData,
  getReportData,
  encryptKingdeeSecret,
  decryptKingdeeSecret,
} from "./kingdee-client.mjs";
import { transformKingdeeData } from "./kingdee-data-transformer.mjs";
...
export function createTenantPlatformRouter(deps) {
  return async function handleTenantPlatformRequest(request, response) {
    ...
    // ─── 公开大屏（无需认证） ─────────────────────────────────────────────────
    if (request.method === "GET" && relativePath.startsWith("/public/dashboards/")) {
      const token = relativePath.slice("/public/dashboards/".length).split("/")[0];
      if (!token) {
        sendJson(request, response, 404, { ok: false, error: "not_found" });
        return;
      }
      const dashboard = getDashboardByPublicToken(deps.db, token);
      if (!dashboard) {
        sendJson(request, response, 404, { ok: false, error: "not_found" });
        return;
      }
      const charts = listDashboardCharts(deps.db, dashboard.id);
      sendJson(request, response, 200, { ok: true, data: { dashboard, charts } });
      return;
    }

    // ─── 金蝶连接配置（tenant_admin） ────────────────────────────────────────
    if (request.method === "POST" && relativePath === "/kingdee/connection") {
      const session = requireSession(request, response, deps);
      if (!session || !requireRole(request, response, session, ["tenant_admin"])) {
        return;
      }
      try {
        const body = await readJsonBody(request);
        const { serverUrl, username, appId, appSecret, accountId } = body;
        if (!serverUrl || !appId || !appSecret || !accountId) {
          sendJson(request, response, 400, { ok: false, error: "missing_fields" });
          return;
        }
        const encryptionKey = deps.config.sessionSecret || "default-key-32-chars-minimum!!!";
        const appSecretEncrypted = encryptKingdeeSecret(appSecret, encryptionKey);
        upsertKingdeeConnection(deps.db, {
          tenantId: session.tenantId,
        ...
        sendJson(request, response, 200, { ok: true, data: getKingdeeConnection(deps.db, session.tenantId) });
      } catch (error) {
    ...
    if (request.method === "GET" && relativePath === "/kingdee/connection") {
      const session = requireSession(request, response, deps);
      if (!session || !requireRole(request, response, session, ["tenant_admin", "member"])) {
        return;
      }
      const conn = getKingdeeConnection(deps.db, session.tenantId);
      sendJson(request, response, 200, { ok: true, data: conn });
      return;
    }

    if (request.method === "POST" && relativePath === "/kingdee/test-connection") {
      const session = requireSession(request, response, deps);
      if (!session || !requireRole(request, response, session, ["tenant_admin"])) {
        return;
      }
      try {
        ...
        const { serverUrl, username, appId, appSecret, accountId } = body;
        const encryptionKey = deps.config.sessionSecret || "default-key-32-chars-minimum!!!";
        const stored = getKingdeeConnectionWithSecret(deps.db, session.tenantId);
        const secret = appSecret ||
          (stored ? decryptKingdeeSecret(stored.app_secret_encrypted, encryptionKey) : null);
        if (!secret) {
          sendJson(request, response, 400, { ok: false, error: "no_credentials" });
          return;
        }
        const conn = getKingdeeConnection(deps.db, session.tenantId);
        await kingdeeAuthenticate({
          serverUrl: String(serverUrl || "").trim() || conn?.serverUrl,
          username: String(username || "").trim() || stored?.username || conn?.username || "",
          appId: String(appId || "").trim() || conn?.appId,
          appSecret: secret,
          accountId: String(accountId || "").trim() || conn?.accountId,
        });
        markKingdeeConnectionTested(deps.db, session.tenantId, true);
        sendJson(request, response, 200, { ok: true, data: { connected: true } });
      } catch (error) {
        markKingdeeConnectionTested(deps.db, session.tenantId, false);
        sendJson(request, response, 200, { ok: true, data: { connected: false, error: error instanceof Error ? error.message : String(error) } });
      }
      return;
    }

    if (request.method === "GET" && relativePath === "/kingdee/report-templates") {
      const session = requireSession(request, response, deps);
      if (!session) {
        return;
      }
      sendJson(request, response, 200, {
        ok: true,
        data: [
          { id: "profitLoss", name: "利润表", description: "收入、成本、利润分析", charts: ["bar", "line"] },
          { id: "balanceSheet", name: "资产负债表", description: "资产、负债、权益分布", charts: ["pie"] },
          { id: "accountsReceivable", name: "应收账款", description: "账龄分析、客户分布", charts: ["pie", "bar"] },
          { id: "accountsPayable", name: "应付账款", description: "账龄分析、供应商分布", charts: ["bar", "pie"] },
          { id: "cashFlow", name: "现金流量", description: "流入流出分析", charts: ["bar", "line"] },
          { id: "custom", name: "自定义查询", description: "自由组合金蝶数据", charts: ["bar", "line", "pie"] },
        ],
      });
      return;
    }

    if (request.method === "POST" && relativePath === "/kingdee/query") {
      const session = requireSession(request, response, deps);
      if (!session || !requireRole(request, response, session, ["tenant_admin", "member"])) {
        return;
      }
      try {
        const body = await readJsonBody(request);
        const { reportId, chartType, formId, fieldKeys, filterString, options } = body;

        const cacheHash = JSON.stringify({ tenantId: session.tenantId, reportId, chartType, formId, fieldKeys, filterString });
        const cached = getKingdeeQueryCache(deps.db, session.tenantId, cacheHash);
        if (cached) {
          sendJson(request, response, 200, { ok: true, data: cached });
          return;
        }
        ...
        const config = stored
          ? {
              ...
              appId: stored.app_id,
              appSecret: decryptKingdeeSecret(stored.app_secret_encrypted, encryptionKey),
              accountId: stored.account_id,
            }
          : { mockMode: true };

        let rawData;
        if (reportId && reportId !== "custom") {
          rawData = await getReportData(config, reportId, { fieldKeys, filterString });
        } else {
          rawData = await queryBillData(config, { FormId: formId || "GL_BALANCESHEET", FieldKeys: fieldKeys || "FID", FilterString: filterString || "" });
        }

        const echartsOption = transformKingdeeData(reportId || "custom", rawData, chartType || "auto", options || {});
        const result = { echartsOption, rawData };
        setKingdeeQueryCache(deps.db, session.tenantId, cacheHash, result, 300);
        sendJson(request, response, 200, { ok: true, data: result });
      } catch (error) {
        sendJson(request, response, 400, { ok: false, error: error instanceof Error ? error.message : String(error) });
      }
    ...
    // ─── 大屏 CRUD ────────────────────────────────────────────────────────────
    if (request.method === "GET" && relativePath === "/dashboards") {
      const session = requireSession(request, response, deps);
      if (!session) {
        return;
      }
      const items = listDashboards(deps.db, session.tenantId, session.userId);
      sendJson(request, response, 200, { ok: true, data: items });
      return;
    }

    if (request.method === "POST" && relativePath === "/dashboards") {
      const session = requireSession(request, response, deps);
      if (!session) {
        return;
      }
      try {
        const body = await readJsonBody(request);
        const dashboard = createDashboard(deps.db, {
          tenantId: session.tenantId,
          userId: session.userId,
          ...body,
        });
        sendJson(request, response, 200, { ok: true, data: dashboard });
      } catch (error) {
        sendJson(request, response, 400, { ok: false, error: error instanceof Error ? error.message : String(error) });
      }
      return;
    ...
    if (dashboardMatch) {
      ...
      if (subPath === "/charts") {
        ...
        if (request.method === "GET") {
          const dashboard = getDashboard(deps.db, dashboardId);
          if (!dashboard || dashboard.tenantId !== session.tenantId) {
            sendJson(request, response, 404, { ok: false, error: "not_found" });
            return;
          }
          sendJson(request, response, 200, { ok: true, data: listDashboardCharts(deps.db, dashboardId) });
          return;
        }
        if (request.method === "POST") {
          ...
          try {
            const body = await readJsonBody(request);
            const chart = createDashboardChart(deps.db, { dashboardId, ...body });
            sendJson(request, response, 200, { ok: true, data: chart });
          } catch (error) {
            sendJson(request, response, 400, { ok: false, error: error instanceof Error ? error.message : String(error) });
          }
          return;
      ...
      const chartMatch = subPath.match(/^\/charts\/([^/]+)$/);
      if (chartMatch) {
        ...
        if (request.method === "PUT") {
          try {
            const body = await readJsonBody(request);
            const chart = updateDashboardChart(deps.db, chartId, body);
            sendJson(request, response, 200, { ok: true, data: chart });
          } catch (error) {
            sendJson(request, response, 400, { ok: false, error: error instanceof Error ? error.message : String(error) });
          }
        ...
        if (request.method === "DELETE") {
          deleteDashboardChart(deps.db, chartId);
          sendJson(request, response, 200, { ok: true });
          return;
        }
      }
      ...
      if (subPath === "/share") {
        ...
        if (request.method === "POST") {
          try {
            ...
            const token = setDashboardPublic(deps.db, dashboardId, isPublic);
            sendJson(request, response, 200, { ok: true, data: { isPublic, publicToken: token } });
          } catch (error) {
            sendJson(request, response, 400, { ok: false, error: error instanceof Error ? error.message : String(error) });
          }
          return;
      ...
      if (!subPath) {
        ...
        if (request.method === "GET") {
          ...
          const charts = listDashboardCharts(deps.db, dashboardId);
          sendJson(request, response, 200, { ok: true, data: { ...dashboard, charts } });
          return;
        }
        if (request.method === "PUT") {
          ...
          try {
            const body = await readJsonBody(request);
            const updated = updateDashboard(deps.db, dashboardId, body);
            sendJson(request, response, 200, { ok: true, data: updated });
          } catch (error) {
            sendJson(request, response, 400, { ok: false, error: error instanceof Error ? error.message : String(error) });
          }
          return;
        ...
        if (request.method === "DELETE") {
          ...
          deleteDashboard(deps.db, dashboardId);
          sendJson(request, response, 200, { ok: true });
          return;
        }
      }
