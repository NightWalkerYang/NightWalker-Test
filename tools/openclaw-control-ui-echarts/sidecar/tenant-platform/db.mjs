...
const MIGRATION_DASHBOARDS_PATH = new URL("./migrations/002_dashboards.sql", import.meta.url);
...
export function openTenantPlatformDb(config) {
  ...
  db.exec(fs.readFileSync(MIGRATION_DASHBOARDS_PATH, "utf8"));
...
// ─── Kingdee Connection CRUD ─────────────────────────────────────────────────

export function upsertKingdeeConnection(db, params) {
  const existing = db
    .prepare("SELECT id FROM kingdee_connections WHERE tenant_id = ?")
    .get(params.tenantId);
  const now = nowIso();
  if (existing) {
    db.prepare(
      `UPDATE kingdee_connections
      ...
      appId: params.appId,
      appSecretEncrypted: params.appSecretEncrypted,
      accountId: params.accountId,
      updatedAt: now,
    });
    return existing.id;
  }
  const id = createId("kconn");
  db.prepare(
    `INSERT INTO kingdee_connections (id, tenant_id, server_url, username, app_id, app_secret_encrypted, account_id, status, created_at, updated_at)
     VALUES (@id, @tenantId, @serverUrl, @username, @appId, @appSecretEncrypted, @accountId, 'active', @createdAt, @updatedAt)`,
  ).run({
    id,
    tenantId: params.tenantId,
    serverUrl: params.serverUrl,
...
export function getKingdeeConnection(db, tenantId) {
  return (
    db
      .prepare(
        `SELECT id, tenant_id AS tenantId, server_url AS serverUrl,
                username, app_id AS appId,
                account_id AS accountId, status, last_tested_at AS lastTestedAt,
                created_at AS createdAt, updated_at AS updatedAt
         FROM kingdee_connections WHERE tenant_id = ?`,
      )
      .get(tenantId) ?? null
  );
}

export function getKingdeeConnectionWithSecret(db, tenantId) {
  return (
    db
      .prepare("SELECT * FROM kingdee_connections WHERE tenant_id = ?")
      .get(tenantId) ?? null
  );
}

export function markKingdeeConnectionTested(db, tenantId, success) {
  db.prepare(
    `UPDATE kingdee_connections
...
export function getKingdeeQueryCache(db, tenantId, queryHash) {
  const row = db
    .prepare(
      `SELECT response_json AS responseJson, expires_at AS expiresAt
       FROM kingdee_query_cache
       WHERE tenant_id = ? AND query_hash = ? AND expires_at > ?`,
    )
    .get(tenantId, queryHash, nowIso());
  return row ? JSON.parse(row.responseJson) : null;
}

export function setKingdeeQueryCache(db, tenantId, queryHash, responseData, ttlSeconds = 300) {
  const now = nowIso();
  const expiresAt = new Date(Date.now() + ttlSeconds * 1000).toISOString();
  db.prepare(
    `INSERT INTO kingdee_query_cache (id, tenant_id, query_hash, response_json, expires_at, created_at)
     VALUES (@id, @tenantId, @queryHash, @responseJson, @expiresAt, @createdAt)
...
export function createDashboard(db, params) {
  const id = createId("dash");
  const now = nowIso();
  db.prepare(
    `INSERT INTO dashboards (id, tenant_id, user_id, title, description, dashboard_type, layout_json, theme, refresh_interval_seconds, is_public, created_at, updated_at)
     VALUES (@id, @tenantId, @userId, @title, @description, @dashboardType, @layoutJson, @theme, @refreshIntervalSeconds, 0, @createdAt, @updatedAt)`,
  ).run({
    id,
    tenantId: params.tenantId,
    userId: params.userId,
    title: String(params.title || "").trim() || "新大屏",
    description: String(params.description || "").trim() || null,
    dashboardType: String(params.dashboardType || "custom").trim(),
    layoutJson: JSON.stringify(params.layout || {}),
    theme: String(params.theme || "dark").trim(),
    refreshIntervalSeconds: Number.isFinite(Number(params.refreshIntervalSeconds))
      ? Number(params.refreshIntervalSeconds)
      : 300,
    createdAt: now,
    updatedAt: now,
  });
...
export function getDashboard(db, id) {
  const row =
    db
      .prepare(
        `SELECT id, tenant_id AS tenantId, user_id AS userId, title, description,
                dashboard_type AS dashboardType, layout_json AS layoutJson, theme,
                refresh_interval_seconds AS refreshIntervalSeconds,
                is_public AS isPublic, public_token AS publicToken,
                created_at AS createdAt, updated_at AS updatedAt
  ...
  return {
    ...row,
    layout: safeParseJson(row.layoutJson, {}),
    isPublic: Boolean(row.isPublic),
  };
}

export function getDashboardByPublicToken(db, token) {
  const row =
    db
      .prepare(
        `SELECT id, tenant_id AS tenantId, user_id AS userId, title, description,
                ...
                is_public AS isPublic, public_token AS publicToken,
                created_at AS createdAt, updated_at AS updatedAt
         FROM dashboards WHERE public_token = ? AND is_public = 1`,
      )
...
export function listDashboards(db, tenantId, userId) {
  return db
    .prepare(
      `SELECT id, tenant_id AS tenantId, user_id AS userId, title, description,
              dashboard_type AS dashboardType, theme,
              refresh_interval_seconds AS refreshIntervalSeconds,
              is_public AS isPublic, public_token AS publicToken,
              created_at AS createdAt, updated_at AS updatedAt
       FROM dashboards
    ...
    .all(tenantId, userId)
    .map((row) => ({ ...row, isPublic: Boolean(row.isPublic) }));
}

export function updateDashboard(db, id, params) {
  const now = nowIso();
  const updates = [];
  const values = { id, updatedAt: now };

  if (params.title !== undefined) {
    updates.push("title = @title");
    values.title = String(params.title || "").trim() || "新大屏";
  }
  if (params.description !== undefined) {
    updates.push("description = @description");
    values.description = String(params.description || "").trim() || null;
  }
  if (params.layout !== undefined) {
    updates.push("layout_json = @layoutJson");
    values.layoutJson = JSON.stringify(params.layout);
  }
  if (params.theme !== undefined) {
    updates.push("theme = @theme");
    values.theme = String(params.theme || "dark").trim();
  }
  if (params.refreshIntervalSeconds !== undefined) {
    updates.push("refresh_interval_seconds = @refreshIntervalSeconds");
    values.refreshIntervalSeconds = Number(params.refreshIntervalSeconds);
  }
  ...
  db.prepare(`UPDATE dashboards SET ${updates.join(", ")} WHERE id = @id`).run(values);
  return getDashboard(db, id);
}

export function setDashboardPublic(db, id, isPublic) {
  const now = nowIso();
  if (isPublic) {
    const token = crypto.randomUUID().replace(/-/g, "");
    db.prepare(
      `UPDATE dashboards SET is_public = 1, public_token = @token, updated_at = @updatedAt WHERE id = @id`,
    ).run({ id, token, updatedAt: now });
    return token;
  }
  db.prepare(
    `UPDATE dashboards SET is_public = 0, public_token = NULL, updated_at = @updatedAt WHERE id = @id`,
...
export function listDashboardCharts(db, dashboardId) {
  return db
    .prepare(
      `SELECT id, dashboard_id AS dashboardId, title, chart_type AS chartType,
              echarts_option_json AS echartsOptionJson, data_query_json AS dataQueryJson,
              position_json AS positionJson, created_at AS createdAt, updated_at AS updatedAt
       FROM dashboard_charts WHERE dashboard_id = ? ORDER BY created_at ASC`,
    )
    .all(dashboardId)
    .map((row) => ({
      ...row,
      echartsOption: safeParseJson(row.echartsOptionJson, {}),
      dataQuery: safeParseJson(row.dataQueryJson, null),
      position: safeParseJson(row.positionJson, { x: 0, y: 0, w: 6, h: 4 }),
    }));
}

export function createDashboardChart(db, params) {
  const id = createId("chart");
  const now = nowIso();
  db.prepare(
    `INSERT INTO dashboard_charts (id, dashboard_id, title, chart_type, echarts_option_json, data_query_json, position_json, created_at, updated_at)
     VALUES (@id, @dashboardId, @title, @chartType, @echartsOptionJson, @dataQueryJson, @positionJson, @createdAt, @updatedAt)`,
  ).run({
    id,
    dashboardId: params.dashboardId,
    title: String(params.title || "").trim() || "图表",
    chartType: String(params.chartType || "custom").trim(),
    echartsOptionJson: JSON.stringify(params.echartsOption || {}),
    dataQueryJson: params.dataQuery ? JSON.stringify(params.dataQuery) : null,
    positionJson: JSON.stringify(params.position || { x: 0, y: 0, w: 6, h: 4 }),
    createdAt: now,
    updatedAt: now,
  });
  return getDashboardChart(db, id);
}

export function getDashboardChart(db, id) {
  const row =
    db
      .prepare(
        `SELECT id, dashboard_id AS dashboardId, title, chart_type AS chartType,
                echarts_option_json AS echartsOptionJson, data_query_json AS dataQueryJson,
                position_json AS positionJson, created_at AS createdAt, updated_at AS updatedAt
         FROM dashboard_charts WHERE id = ?`,
      )
      .get(id) ?? null;
  if (!row) {
    return null;
...
export function updateDashboardChart(db, id, params) {
  const now = nowIso();
  const updates = [];
  const values = { id, updatedAt: now };
  ...
  if (params.title !== undefined) {
    updates.push("title = @title");
    values.title = String(params.title || "").trim() || "图表";
  }
  if (params.echartsOption !== undefined) {
    updates.push("echarts_option_json = @echartsOptionJson");
    values.echartsOptionJson = JSON.stringify(params.echartsOption);
  }
  if (params.dataQuery !== undefined) {
    updates.push("data_query_json = @dataQueryJson");
    values.dataQueryJson = params.dataQuery ? JSON.stringify(params.dataQuery) : null;
  }
  if (params.position !== undefined) {
    updates.push("position_json = @positionJson");
    values.positionJson = JSON.stringify(params.position);
  }
  ...
  updates.push("updated_at = @updatedAt");
  db.prepare(`UPDATE dashboard_charts SET ${updates.join(", ")} WHERE id = @id`).run(values);
  return getDashboardChart(db, id);
}

export function deleteDashboardChart(db, id) {
  db.prepare("DELETE FROM dashboard_charts WHERE id = ?").run(id);
}
