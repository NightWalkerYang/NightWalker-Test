...
def build_parser() -> argparse.ArgumentParser:
    ...
    default_project_root = str(
        tenant_profile.get("analyticsProjectRoot") or DEFAULT_PROJECT_ROOT
    ).strip() or DEFAULT_PROJECT_ROOT
    default_catalog_root = str(
        tenant_profile.get("catalogRoot") or DEFAULT_CATALOG_ROOT
    ).strip() or DEFAULT_CATALOG_ROOT

    parser = argparse.ArgumentParser(
        description="Perform controlled writes or host CLI operations against the deployed Kingdee analytics runtime.",
    )
    ...
    operation.add_argument(
        "--cli",
        choices=("init-db", "sync-object", "sync-sales-module"),
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="repeatable object-code selector for sync-object or sync-sales-module",
    )
    return parser
...
def build_cli_args(args: argparse.Namespace) -> list[str]:
    ...
    if cli_command == "sync-sales-module":
        cli_args = ["sync-sales-module", "--default-start", args.default_start]
