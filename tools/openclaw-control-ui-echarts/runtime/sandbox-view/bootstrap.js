...
export function buildSandboxViewPublicBootstrapTag(options = {}) {
  const runtimeAssetBasePath = String(options.runtimeAssetBasePath || "/assets/runtime").trim();
  return `    <script type="module" src="${runtimeAssetBasePath}/sandbox-view/preboot.js" ${SANDBOX_VIEW_BOOTSTRAP_MARKER}></script>`;
}

export function injectSandboxViewPublicBootstrap(indexHtml, options = {}) {
  return injectBeforeMainBundle(indexHtml, buildSandboxViewPublicBootstrapTag(options));
}
