...
- Tenant-admin Agent assignment now opens a member-scoped `撤回分配` dialog that lists the selected member's assigned Agents with per-Agent multi-select and select-all; the sidecar marks matching `user_agent_assignments` rows inactive so member Agent lists and assignment counts stay in sync without deleting history, and the display name resolution now prefers catalog names, then assignment descriptions, instead of surfacing raw placeholder values such as `not_found`.
