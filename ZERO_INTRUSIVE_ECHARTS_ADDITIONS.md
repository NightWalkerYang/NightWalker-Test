...
- Tenant-admin, platform-admin, and member Agent-selection list feedback now uses a shared auto-dismissing floating toast instead of an inline bottom callout, and the native-shell `已就绪` hints follow the same toast treatment.
