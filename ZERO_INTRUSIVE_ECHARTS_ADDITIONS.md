...
- Tenant members now click assigned Agent cards into the native `/chat` page, where a zero-intrusive sidebar adds `新建会话`, a per-Agent session list, and front-end-only session hiding with delete confirmation, while `Agent选择` moves into the native top breadcrumb area and the native chat content and features remain intact.
