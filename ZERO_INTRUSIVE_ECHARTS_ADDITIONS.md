...
- Tenant admins can now delete members directly from the member list through a zero-intrusive confirmation dialog; deletion uses logical removal, hides the member from tenant/platform member lists, marks the member account inactive, automatically invalidates that member's active Agent assignments, and clears that member's derived Agent workspace directories while preserving history records in the database. Recreating the same username inside that same tenant now reactivates the logically deleted member record and refreshes the password instead of failing on the historical `users.username` uniqueness constraint.
