...
- `tools/openclaw-control-ui-echarts/runtime/tenant/tenant-console-agents.js`
- `tools/openclaw-control-ui-echarts/runtime/tenant/tenant-console-usage.js`
- `tools/openclaw-control-ui-echarts/runtime/tenant/tenant-console-events.js`
...
- Fenced `image-upload` blocks render as single-card multi-slot upload forms (not generic form DSL): each slot validates `png/jpeg/webp`, keeps the selected file local for preview first, and only uploads through member-auth `multipart/form-data` after the user clicks `上传完成并继续`; successful uploads persist only into the current member's derived workspace under `Echarts/**`, while the continuation prompt still includes explicit `slot -> Echarts/...` plus `./assets/...` hints. Once that continuation message is sent successfully, the card flips into a completed visual state and locks further upload/send interactions to prevent duplicate submissions.
- Fenced `single-select` and `multi-select` blocks now render as inline option cards. Users can select one or more suggested next steps and send the composed prompt directly back to the chat flow. Once that send succeeds, the card flips into a completed visual state and locks further interaction to prevent duplicate sends.
...
- Those helpers now also inspect the current local gateway image's `/app/dist/.buildstamp`; when the image trails the current checkout but the Git diff only touches zero-intrusive files, they reuse that image's upstream Control UI instead of rebuilding the whole gateway image again before regenerating the overlay.
...
- The tenant sidecar now also syncs each active derived member Agent into runtime `openclaw.json` as a real agent entry with its own `id`, `workspace`, and `agentDir`, and removes that entry again when the assignment is revoked or deleted, so member chat session keys that carry the derived agent id keep resolving to the isolated workspace instead of failing with `Agent ... no longer exists in configuration`.
...
- The same direct-docker setup helpers now also enable `hooks.internal.entries.tenant-member-bootstrap-filter.enabled=true` on the live runtime config, and that sync is wired into both `setup-direct-docker-compose-up.mjs` and the bash deployment entrypoint `setup-direct-docker-compose-up.sh`, so member chat deployments do not regress into the bootstrap self-introduction path just because the server was updated through the shell workflow.
...
- Tenant member chat draft routing is now locked end-to-end: `member-chat-surface.js` records an unresolved per-member draft-session lock in `sessionStorage`, `preboot.js` and its `history.pushState/replaceState` patch honor that lock by stripping the draft `session=` back out of `/chat?tenantAgentId=...`, and the lock is cleared only after the first real member message promotes the session or the draft is abandoned. This keeps the native `/chat` welcome state visible after clicking `New session` instead of regressing into a blank assistant shell or snapping back to an older member session.
...
- Tenant-admin Skills workbench now removes the route-level viewport-height cap and lets the selected Agent skill cards participate in the native content page flow, so large skill sets remain reachable after scrolling to the bottom instead of being clipped by an inner workbench viewport.
