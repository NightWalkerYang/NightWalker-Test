...
- Tenant-admin Agent assignment now supports row-level and batch `撤回分配`; the sidecar marks matching `user_agent_assignments` rows inactive so member Agent lists and assignment counts stay in sync without deleting history.
