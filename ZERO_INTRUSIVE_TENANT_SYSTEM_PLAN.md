...
7. 租户管理员基础能力已落地
   ...
   - 原生侧边栏会按租户管理员角色裁剪，只保留：
     ...
     - `耗量统计`
     - 版本信息
   - 耗量统计页以列表形态展示 `tenant_wallet_ledger` 中 `direction='debit'` 且 `category ∈ ('usage','consume')` 的消耗记录，顶部搜索可按成员账号或 Agent 名模糊匹配，底部提供服务端分页；表格列为 `时间 / 成员账号 / Agent / 消耗积分 / Tokens / 备注`
   - 耗量统计页的数据写入依赖后续步骤 8 的 Agent 使用后扣费链路，当前扣费写入尚未接通时列表以空态 `暂无消耗记录` 展示
