...
- 对草稿态或空历史会话，零侵入 runtime 不能直接把原生聊天区渲染成“空消息数组 + 非 loading”；实测在 `172.30.31.203` 这类 cloud bundle 环境会把原生聊天页卡死，必须先用可渲染的空占位消息兜住，再等待真实消息或后续历史回填
- 只看到当前 Agent 剩余积分提示
...
8. 公有云模式支付与扣费已落地
   - 账号密码登录
    ...
    - 租户管理员原生侧边栏已新增 `钱包 -> 钱包充值 / 充值订单 / 钱包流水`
    - `充值订单` 与 `钱包流水` 当前已拆成两个独立 list 页面，并统一补上搜索框与分页
    - `划转到 Agent` 当前已从钱包页移到 `Agent -> 已有Agent` 卡片内，并统一更名为 `划转积分`
