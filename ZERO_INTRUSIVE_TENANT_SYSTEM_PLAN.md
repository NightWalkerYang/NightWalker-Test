...
1. 原生单入口登录视图已落地
   ...
   - 成员聊天路由修正已前移到原生 Control UI 主 bundle 之前：零侵入构建现在会在 `index-*.js` 前注入 `runtime/tenant/preboot.js`，先把 `tenantAgentId/session` 与 Control UI 本地 settings 预种成成员上下文，避免 `172.30.31.203` 这类环境先用 `main` session 启动再晚到成员 session 时把聊天页拖死
